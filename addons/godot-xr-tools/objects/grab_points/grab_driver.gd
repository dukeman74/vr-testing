...
func _physics_process(delta : float) -> void:
	...
	if primary.hand_point is XRToolsGrabPointHand:
		primary.by.rotation+=primary.hand_point.angle_add
		primary.by.position+=primary.hand_point.position_add
	...
	if is_instance_valid(secondary):
		...
		var x1 := destination
		var x2 : Transform3D
		if secondary.hand_point is XRToolsGrabPointHand:
			secondary.by.rotation+=secondary.hand_point.angle_add
			secondary.by.position+=secondary.hand_point.position_add
